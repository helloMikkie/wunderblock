<div class="consent-gate-container" data-consent-needed="gate"
    data-vendor="<?= strtolower(option('fendinger.consent-gate.vendors')[$vendor]['name']) ?>"
    data-consent-processed="true">
    <div class="consent-gate" data-role="consent-gate">
        ...
        <div class="consent-gate-vendor">
            ...
            <span
                class="consent-gate-vendor-name"><?= option('fendinger.consent-gate.vendors')[$vendor]['name'] ?></span>
        </div>
        ...
        <div class="consent-gate-toggle" data-role="consent-gate-toggle">
            <div class="toggle-off">
                <svg class="icon consent-gate-icon-toggle-off" xmlns="http://www.w3.org/2000/svg" height="48"
            ...
            <div class="toggle-on">
                ...
                <a data-role="consent-gate-privacy" class="consent-gate-privacy"
                    href="<?= option('fendinger.consent-gate.vendors')[$vendor]['privacy'] ?>"
                    target="_blank"><?= tt('fendinger.consent-gate.privacy', ['vendor' => option('fendinger.consent-gate.vendors')[$vendor]['name']]) ?></a>
            </div>
        </div>
        <p class="consent-gate-text">
            <?= tt('fendinger.consent-gate.agreement', ['vendor' => option('fendinger.consent-gate.vendors')[$vendor]['name']]) ?><br>
            <?= tt('fendinger.consent-gate.more', ['vendor' => option('fendinger.consent-gate.vendors')[$vendor]['name']]) ?>
            <a data-role="consent-gate-privacy" class="consent-gate-privacy"
                href="<?= option('fendinger.consent-gate.vendors')[$vendor]['privacy'] ?>"
                title="<?= tt('fendinger.consent-gate.privacy', ['vendor' => option('fendinger.consent-gate.vendors')[$vendor]['name']]) ?>"
                target="_blank"><?= tt('fendinger.consent-gate.privacy', ['vendor' => option('fendinger.consent-gate.vendors')[$vendor]['name']]) ?></a>.
        </p>
    </div>
    <script data-role="consent-gate-template" data-content=""></script>
    <script data-role="consent-gate-content-template"
        data-content="<?= htmlentities('<div class="oembed-content">' . $content . '</div>') ?>"></script>
</div>
<?php
